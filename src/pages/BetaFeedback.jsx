...
if (q.type === 'checkbox') return (<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{q.options.map(o => (<label key={o} className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={((value || [])).includes(o)} onChange={() => { const cur = (value || []); onChange(q.key, cur.includes(o)? cur.filter(x => x!== o): [...cur, o]); }} className="accent-primary" />{o}</label>))}</div>);
